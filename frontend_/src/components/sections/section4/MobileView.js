import { faArrowCircleRight, faGraduationCap, faUserCircle} from "@fortawesome/free-solid-svg-icons";
...
const InformationBlock = ({title, text, obj, obj:{type, role}}) => {
    ...
    return (
        ...
        className={
            ...
            `}>
            <Logo logo={obj.logo} />
            <div className={`flex flex-col justify-start items-start leading-6 tracking-wide`}>
    ...
    return (
        <div className="w-full h-fit">
            <div className={`h-fit w-fit flex flex-col justify-center items-start`}>
                ...
                <ol className={`font-normal ${dark ? 'text-slate-800': 'text-slate-600'} text-[0.85rem]  leading-6 tracking-wide p-2`}>
